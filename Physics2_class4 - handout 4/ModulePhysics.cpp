...
PhysBody* ModulePhysics::CreateCircle(int x, int y, int radius,b2BodyType btype,int type, bool bullet, float restitution)
{
	b2BodyDef body;
	body.type = btype;
...
PhysBody* ModulePhysics::CreateChain(int x, int y, int* points, int size, b2BodyType btype,int type, bool open_chain, float restitution)
{
	b2BodyDef body;
	body.type = btype;
...
update_status ModulePhysics::PostUpdate()
{
	...
	for(b2Body* b = world->GetBodyList(); b; b = b->GetNext())
	{
		for(b2Fixture* f = b->GetFixtureList(); f; f = f->GetNext())
		{
			...
			if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_DOWN)
			{
				...
				while (fixture != NULL)
				{
					if (fixture->GetShape()->TestPoint(b->GetTransform(), p))
					{
						found_body = b;
					}
	...
	if (found_body != nullptr)
	{
		b2MouseJointDef def;
		def.bodyA = ground;
		def.bodyB = found_body;
		def.target = p;
		def.dampingRatio = 0.5f;
		def.frequencyHz = 2.0f;
		def.maxForce = 100.0f * found_body->GetMass();
	...
	if (App->input->GetMouseButton(SDL_BUTTON_LEFT) == KEY_REPEAT && mouse_joint != nullptr)
	{
		...
		App->renderer->DrawLine(METERS_TO_PIXELS(p.x), METERS_TO_PIXELS(p.y), METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().x), METERS_TO_PIXELS(mouse_joint->GetBodyB()->GetPosition().y), 255, 0, 0);
