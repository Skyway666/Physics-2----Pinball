...
void ModuleFonts::BlitText(int x, int y, int font_id, const char* text) const
{
	...
	for (uint i = 0; i < len; ++i)
	{
		...
		while (counter != font->row_chars)
		{
			if (fonts[font_id].table[counter] == text[i])
			{
				rect.y = font->char_h * (font_id);
				rect.x = font->char_w * counter;
				App->renderer->Blit(fonts[font_id].graphic, x, y, 1, &rect, 1.0f, 0, false, INT_MAX, INT_MAX, font->char_w * 3, font->char_h * 3);
				x += font->char_w * 3;
				counter = 0;
				break;
			}
